...
const TW_PAD: Record<string, string> = {
  sm: 'px-3 py-1.5',
  md: 'px-4 py-2',
  lg: 'px-6 py-3',
};
...
const TW_RAD: Record<string, string> = {
  none: 'rounded-none',
  sm: 'rounded',
  md: 'rounded-lg',
  lg: 'rounded-xl',
...
const TW_CARD_PAD: Record<string, string> = {
  sm: 'p-4',
  md: 'p-6',
  lg: 'p-8',
};
const TW_MAXW: Record<string, string> = {
  sm: 'max-w-sm',
  md: 'max-w-md',
  lg: 'max-w-3xl',
  xl: 'max-w-6xl',
...
const TW_CONT_PAD: Record<string, string> = {
  sm: 'px-4 py-6',
  md: 'px-6 py-8',
  lg: 'px-8 py-12',
};
const TW_MODAL_W: Record<string, string> = {
  sm: 'max-w-sm',
  md: 'max-w-lg',
  lg: 'max-w-2xl',
};

const CSS_PAD: Record<string, { x: number; y: number }> = {
  sm: { x: 12, y: 6 },
  md: { x: 16, y: 8 },
  lg: { x: 24, y: 12 },
};
const CSS_FS: Record<string, number> = { sm: 12, md: 14, lg: 16 };
const CSS_RAD: Record<string, string> = {
  none: '0',
  sm: '0.25rem',
  md: '0.5rem',
  lg: '0.75rem',
...
const CSS_SHADOW: Record<string, string> = {
  none: 'none',
...
const CSS_MAXW: Record<string, string> = {
  sm: '384px',
  md: '448px',
  lg: '768px',
  xl: '1152px',
...
const CSS_CONT_PAD: Record<string, string> = {
  sm: '24px 16px',
  md: '32px 24px',
  lg: '48px 32px',
};
const CSS_MODAL_W: Record<string, string> = {
  sm: '384px',
  md: '512px',
  lg: '672px',
};
...
export function generateButtonCode(
  ...
  switch (c.variant) {
    case 'primary':
      variant = `bg-[${d.primaryColor}] text-${txtCol} hover:opacity-90`;
      break;
    case 'secondary':
      variant = `bg-[${d.primaryColor}]/10 text-[${d.primaryColor}] hover:bg-[${d.primaryColor}]/20`;
      break;
    case 'outline':
      variant = `border border-[${d.primaryColor}]/30 text-[${d.primaryColor}] hover:bg-[${d.primaryColor}]/5`;
      break;
    case 'ghost':
      variant = `text-[${d.primaryColor}] hover:bg-[${d.primaryColor}]/10`;
      break;
    case 'destructive':
  ...
  const imp = c.icon
    ...
    : '';
  const iL =
    c.icon && c.iconPosition === 'left'
      ? `\n      <${c.icon} size={${sz}} />`
      : '';
  const iR =
    c.icon && c.iconPosition === 'right'
      ? `\n      <${c.icon} size={${sz}} />`
      : '';
  ...
  switch (c.variant) {
    case 'primary':
      bg = d.primaryColor;
      col = contrast(d.primaryColor);
      hov = 'opacity: 0.9;';
      bdr = 'border: none;';
      break;
    case 'secondary':
      bg = `${d.primaryColor}1a`;
      col = d.primaryColor;
      hov = `background-color: ${d.primaryColor}33;`;
      bdr = 'border: none;';
      break;
    case 'outline':
      bg = 'transparent';
      col = d.primaryColor;
      hov = `background-color: ${d.primaryColor}0d;`;
      bdr = `border: 1px solid ${d.primaryColor}4d;`;
      break;
    case 'ghost':
      bg = 'transparent';
      col = d.primaryColor;
      hov = `background-color: ${d.primaryColor}1a;`;
      bdr = 'border: none;';
      break;
    case 'destructive':
      bg = '#ef4444';
      col = '#ffffff';
      hov = 'background-color: #dc2626;';
      bdr = 'border: none;';
      break;
  }
...
export function generateCardCode(
  ...
  const imgCSS = c.hasImage
    ? `\n  .card-image {\n    height: 192px;\n    background: linear-gradient(135deg, ${d.primaryColor}1a, ${d.primaryColor}4d);\n  }\n`
    : '';
  const imgHTML = c.hasImage ? '\n  <div class="card-image"></div>' : '';
  const actionCSS = c.hasActions
    ? `\n  .card-action {\n    margin-top: 1rem;\n    padding: 8px 16px;\n    background-color: ${d.primaryColor};\n    color: ${contrast(d.primaryColor)};\n    font-size: 14px;\n    font-weight: 500;\n    border-radius: 0.5rem;\n    border: none;\n    cursor: pointer;\n  }\n`
    : '';
...
export function generateContainerCode(
  ...
  const border = c.hasBorder ? 'border border-zinc-200' : '';
  const cls = ['mx-auto', mw, pad, bgMap[c.background], border]
    .filter(Boolean)
    .join(' ');
  ...
  const bgCSS: Record<string, string> = {
    transparent: 'transparent',
    white: '#ffffff',
    light: '#fafafa',
    dark: '#18181b',
  };
  const textCSS = c.background === 'dark' ? '#ffffff' : '#18181b';
  ...
  .container {
    ...
    margin: 0 auto;
    padding: ${cpad};
    background-color: ${bgCSS[c.background]};
    color: ${textCSS};${borderCSS}
  }
</style>
